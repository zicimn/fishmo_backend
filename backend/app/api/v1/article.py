...
async def visit(
    ...
    # 先获取文章和作者信息
    base_stmt = (
        select(Article, User.username,User.avatar,User.id).join(User, Article.author_id == User.id).where(Article.id == id)
    )
    ...
    article, author_name, author_avatar, author_id = row

    if article.status == False and (not credentials or verify_login(credentials) != author_id):
        raise HTTPException(403,"文章不可见")
...
async def index(
    ...
    # 构建基础查询
    base_stmt = (
        select(Article.id, Article.title, Article.author_id, Article.category, 
               Article.views, Article.likes, Article.created_at, User.username,User.avatar)
    ...
    for article_id, title, author_id, category, views, likes, created_at, name, avatar in rows:
        items.append(
            ArticleItem(
                id=article_id,
                title=title,
...
async def edit(id :int,
    ...
    if data.status is not None:#修改状态
    ...
    #由于前端传的images默认是[],而不是None,所以单独处理判断
    if not data.images:#如果前端传入了images为[],表示清空图片
        print(f"[DEBUG] CLEAR IMAGES, data.imagses: {data.images}")
        article.images = []

...
const handleSubmit = async () => {
  ...
  try {
    ...
    // Only send images if user modified them
  ...
  <div class="edit-page">
    ...
    <div v-if="loading && !error" class="loading-state">
      <div class="loading-card">
        ...
        <span class="loading-text">加载文章...</span>
      </div>
    </div>

    <!-- Error state (no form displayed) -->
    <div v-else-if="error && !title" class="error-page">
      <div class="error-card">{{ error }}</div>
    </div>

    <!-- Main editor layout -->
    <div v-else class="edit-layout-wrapper">
          ...
          <div class="sidebar-inner">
            ...
            <div class="sb-section">
              ...
              <div class="status-group">
                <button
                  type="button"
                  :class="['status-btn', { active: articleStatus }]"
                  @click="articleStatus = true"
                  :disabled="loading"
                >
              ...
              <p class="status-hint">{{ articleStatus ? '所有用户可见' : '仅自己可见（隐藏状态）' }}</p>
            </div>
            ...
            <div class="sb-submit-area">
              <button type="button" class="sb-submit-btn" :disabled="loading" @click="handleSubmit">
        ...
        <main class="edit-editor">
          ...
          <div class="editor-content-area">
            <textarea
              ...
              v-model="content"
              :disabled="loading"
              placeholder="编辑文章内容... 支持 Markdown 格式"
...
.sb-submit-btn:active:not(:disabled) {
  transform: scale(0.98);
}
